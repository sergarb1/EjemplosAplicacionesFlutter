...
import 'package:consumo_api/widgets/character_card.dart'; // Widget para mostrar personajes
import 'package:consumo_api/widgets/custom_scaffold.dart'; // Widget personalizado

// Clase para la pantalla que muestra la lista de personajes
class CharactersPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Obtenemos el proveedor de datos para acceder a los personajes
    ...
    return CustomScaffold(
      ...
      body: Column(
        children: [
          ...
          // Si no está cargando y no hay error, mostramos la lista de personajes
          if (!apiProvider.isLoading && apiProvider.error == null)
            Expanded(
              child: ListView.builder(
                padding: const EdgeInsets.all(16.0), // Espacio alrededor
                itemCount: apiProvider.characters.length, // Número de personajes
                itemBuilder: (context, index) {
                  // Crea una tarjeta para cada personaje
                  return CharacterCard(character: apiProvider.characters[index]);
                },
              ),
            ),
          // Si hay una página siguiente, mostramos un botón para cargar más
          if (apiProvider.characterInfo?.next != null)
            Padding(
              padding: const EdgeInsets.all(16.0), // Espacio alrededor
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  final nextPage = int.parse(Uri.parse(apiProvider.characterInfo!.next!).queryParameters['page']!);
                  // Cargamos los personajes de la siguiente página
                  apiProvider.loadCharacters(page: nextPage);
                },
                // Estilo del botón
